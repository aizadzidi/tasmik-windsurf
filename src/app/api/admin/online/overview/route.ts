...
import { isMissingRelationError, monthStartUtc, toDateKey } from "@/lib/online/db";
  ...
  try {
    ...
    const payload = await adminOperationSimple(async (client) => {
      ...
      const { data: courseData, error: courseError } = await client
        ...
        .eq("tenant_id", tenantId)
        .eq("is_active", true);
      if (courseError && !isMissingRelationError(courseError, "online_courses")) {
        throw courseError;
      }
      ...
      if (onDutyError && !isMissingRelationError(onDutyError, "online_teacher_slot_preferences")) {
        throw onDutyError;
      }
      ...
      if (attendanceError && !isMissingRelationError(attendanceError, "online_attendance_sessions")) {
      ...
      if (rollupError && !isMissingRelationError(rollupError, "online_attendance_monthly_rollup")) {
        throw rollupError;
      }
