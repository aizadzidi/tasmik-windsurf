...
type ReportScope = "campus" | "online";
type LatestReport = {
  type: string;
  surah?: string;
  ayat_from?: number;
...
type AdminHafazanReportsPageProps = {
  scope?: ReportScope;
};

export default function AdminHafazanReportsPage({
  scope = "campus",
}: AdminHafazanReportsPageProps) {
  ...
  const fetchStudentProgress = useCallback(async () => {
    ...
    try {
      ...
      const response = await authFetch(
        `/api/admin/reports?viewMode=${viewMode}&scope=${scope}`
      );
          ...
          <header className="mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">
                {scope === "online"
                  ? "Online Hafazan Reports"
                  : "Student Progress Reports"}
              </h1>
              <p className="text-gray-600">
                {scope === "online"
                  ? "Monitor online tasmik, murajaah and juz tests activity."
                  : "Monitor tasmik, murajaah and juz tests activity across all students"}
              </p>
            </div>
          </header>
