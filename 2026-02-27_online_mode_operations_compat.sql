-- Compatibility migration for tenants that already have legacy online_courses tables.
-- This upgrades the existing schema to the online slot claim + attendance model
-- without dropping legacy course data.
...
update public.online_courses
set name = trim(
  coalesce(title, 'Online Course') ||
  case
    when nullif(trim(plan_label), '') is null then ''
    else ' (' || trim(plan_label) || ')'
  end
)
where name is null;

update public.online_courses
set monthly_fee_cents = round(coalesce(price_myr, 0) * 100)::integer
where monthly_fee_cents is null;

update public.online_courses oc
set program_id = (
  select p.id
  from public.programs p
...
begin
  if not exists (
    ...
    where conrelid = 'public.online_courses'::regclass
      and conname = 'online_courses_tenant_id_fkey'
  ) then
    alter table public.online_courses
      add constraint online_courses_tenant_id_fkey
  ...
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'public.online_courses'::regclass
      and conname = 'online_courses_created_by_fkey'
  ) then
    alter table public.online_courses
      add constraint online_courses_created_by_fkey
  ...
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'public.online_courses'::regclass
      and conname = 'online_courses_id_tenant_key'
  ) then
    alter table public.online_courses
      add constraint online_courses_id_tenant_key
      unique (id, tenant_id);
  end if;
  ...
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'public.online_courses'::regclass
      and conname = 'online_courses_monthly_fee_cents_check'
  ) then
    alter table public.online_courses
      add constraint online_courses_monthly_fee_cents_check
...
begin
  if not exists (
    ...
    where conrelid = 'public.online_slot_templates'::regclass
      and conname = 'online_slot_templates_id_tenant_key'
  ) then
    alter table public.online_slot_templates
      add constraint online_slot_templates_id_tenant_key
      unique (id, tenant_id);
  end if;
...
begin
  if not exists (
    ...
    where conrelid = 'public.online_slot_claims'::regclass
      and conname = 'online_slot_claims_id_tenant_key'
  ) then
    alter table public.online_slot_claims
      add constraint online_slot_claims_id_tenant_key
      unique (id, tenant_id);
  end if;
